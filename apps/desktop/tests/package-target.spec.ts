...
describe('desktop package target', () => {
  ...
  it('rejects unsupported targets and hosts before building', () => {
    ...
    expect(() => resolveDesktopPackageTarget('openbsd-x64', 'openbsd', 'x64')).toThrow(/unsupported target/u)
  })
  ...
  it('accepts unsigned Windows and Apple Silicon artifacts and rejects other targets or preparation-only use', () => {
    ...
    expect(() => parseDesktopPackageInvocation(['mac-x64', '--unsigned'], 'darwin', 'x64'))
      .toThrow(/supports win-x64 and mac-arm64/u)

...
| 私有产品插件 | Threerouter 品牌与图文视频工具位于 `plugins/`，在 dsh 核心包集合之外，打包的 dsh 树不会包含它们。 | 打包构建先运行各插件的 bundle 脚本，`prepare:plugins` 再把构建产物复制到 `app.asar/dsh/node_modules`；随包 profile 的 bundles 因此可以离线解析，无需在 profile 中声明依赖。 |

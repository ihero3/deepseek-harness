...
| Private product plugins | The Threerouter brand and image/video tools live in `plugins/`, outside the dsh core package set, so the packaged dsh tree would not contain them. | The packaging build runs each plugin's bundle script and `prepare:plugins` copies the built packages into `app.asar/dsh/node_modules`; the shipped profile bundles then resolve offline without profile dependencies. |
